...
- **Stata** is a powerful app with a user-friendly interface that is used widely by researchers in economics, sociology, political science, and public health. We will use it extensively for data management, analysis, and visualization. [See more in the Stata section](#stata-section).
...
ECN 310 is designed to be accessible to students with zero coding and research experience. That means you are not required or expected to do anything with this information before starting ECN 310.

At the same time, here we provide some resources so you can get comfortable with the tools we use in ECN 310. The easiest first step is to learn about Markdown (instructions here, link to GitHub explainer), which is a set of commands we will use to format text on Github.
...
If you like that and want to go a step further, you can try out LaTeX (instructions here). LaTeX does a lot of the same things as Markdown, but it can do lots more. That is why we use it to write the final papers! These commands are a little more complex than ones used on GitHub. 

Another possibility is to start experimenting withStata. There are instructions (here) to get access to the app, and (here) to follow the textbook we'll use this fall to try out some of what Stata can do.